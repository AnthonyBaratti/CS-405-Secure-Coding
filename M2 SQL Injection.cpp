...
//
// The purpose of this program is to detect and mitigate SQL injection.
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
#include <tuple>
#include <vector>
#include <regex> //included for pattern matching.
...
/////////////////////////////////////////////////
// *NOTE It is worth noting that in all of my research
//       it showed that using parameterized or prepared statements
//       such as using a ? instead of 'Fred' and then connecting
//       to create data rather than code is the best approach
//       but since I can't change the code here, that is a moot
//       prospect to achieve, so regex matching is used here,
//       but is proven to be far less effective. Since the 
//       sql statement is appended rather than from user input,
//       the regex is the only way I could think of to solve this.
/////////////////////////////////////////////////

//Will use regex to match potential sql injections before execution
bool detectSQLInjection(const std::string& sql) {

    //patterns to detect -- not comprehensive
    std::regex detectedPattern(R"((\bor\b|\bAND\b)[^=]*[=<>!][^=]*['\"]?\w+['\"]?)", std::regex::icase);


    ///////////////////////////////////
    // Ref:
    // CPP Reference. (2024). std::regex_search. Cppreference.com.
    // https://en.cppreference.com/w/cpp/regex/regex_search
    ///////////////////////////////////
    

    //if match is found between sql and detectedPattern, return true, else false
    return std::regex_search(sql, detectedPattern); 
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //Runs the string sql (query) through the regex injection matcher.
  if (detectSQLInjection(sql)) {
      //if match found (returns true), display error and abort.
      std::cout << "Potential SQL Injection. Query Denied." << std::endl;
      return false;
  }
